...
Though all of core algorithms written in Java, the _rJava_ package must be firstly installed in the R environment as well. Normally, the dependent package would be also installed by the above command. Otherwise, we should install it manually in a similar way to RMut. After installation, the RMut package can be loaded via

_> library(RMut)_

In addition, we could set the _Maximum Java heap size_ to a large value, for ex., 8GB (in case of large-scale networks analysis) via

_> .jinit(parameters="-Xmx8000m")_
